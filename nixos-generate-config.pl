...
sub pciCheck {
    my $path = shift;
    my $vendor = readfile "$path/vendor"; chomp $vendor;
    my $device = readfile "$path/device"; chomp $device;
    my $class = readfile "$path/class"; chomp $class;
...
sub usbCheck {
    my $path = shift;
    my $class = readfile "$path/bInterfaceClass"; chomp $class;
    my $subclass = readfile "$path/bInterfaceSubClass"; chomp $subclass;
    my $protocol = readfile "$path/bInterfaceProtocol"; chomp $protocol;
...
# devices.
my @swaps = readfile("/proc/swaps", err_mode => 'carp');
my @swapDevices;
if (@swaps) {
    shift @swaps;
...
EOF
    ...
    if ($device =~ /\/dev\/loop(\d+)/) {
        my $loopnr = $1;
        my $backer = readfile "/sys/block/loop$loopnr/loop/backing_file";
        if (defined $backer) {
            chomp $backer;
            $device = $backer;
...
EOF
    ...
    if (-e $device) {
        ...
        my $dmUuid = readfile("/sys/class/block/$deviceName/dm/uuid",  err_mode => 'quiet');
        if ($dmUuid =~ /^CRYPT-LUKS/)
        {
            ...
            if (scalar @slaves == 1) {
                ...
                if (-e $slave) {
                    my $dmName = readfile("/sys/class/block/$deviceName/dm/name");
                    chomp $dmName;
